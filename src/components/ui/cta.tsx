...
export default function CTASection() {
  ...
  return (
    <section id="contact" className="py-24 relative overflow-hidden bg-background scroll-mt-20">
      <div className="max-w-4xl mx-auto px-6 relative z-10 flex flex-col gap-20">
        
        {/* BLOQUE 1: INTRODUCCIÓN Y FOUNDER */}
        <div className="flex flex-col md:flex-row items-center gap-12 md:gap-16">
          <div className="text-center md:text-left flex-1 space-y-6">
            ...
            <div className="flex justify-center md:justify-start">
              <button className="h-14 px-8 text-white bg-gradient-to-r from-blue-600 to-purple-600 rounded-2xl flex items-center shadow-md hover:opacity-90 transition-all font-semibold cursor-pointer">
          ...
          {/* Founder Card al lado (en desktop) o abajo (en móvil), pero integrada en el bloque superior */}
            ...
            <form className="space-y-6" onSubmit={handleSubmit}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <input 
                  ...
                  className="w-full h-14 px-5 border-none rounded-2xl text-foreground placeholder:text-muted-foreground/40 focus:ring-1 focus:ring-blue-500/20 transition-all outline-none" 
                />
                <input 
                  type="email" 
                  required
              ...
              <div className="relative flex gap-3" ref={dropdownRef}>
                <div className="relative">
                  <button
                    ...
                    style={{ backgroundColor: 'var(--cta-input-bg)' }}
                    className="h-14 pl-4 pr-3 rounded-2xl flex items-center gap-2 transition-all outline-none focus:ring-1 focus:ring-blue-500/20 min-w-[110px] justify-between"
                  >
                  ...
                  <AnimatePresence>
                    {isDropdownOpen && (
                      <motion.div
                        ...
                        exit={{ opacity: 0, y: 5 }}
                        className="absolute top-16 left-0 w-[280px] max-h-[300px] bg-background border border-border rounded-xl shadow-2xl z-50 overflow-hidden flex flex-col"
                      >
                        <div className="p-2 border-b border-border sticky top-0 bg-background/95 backdrop-blur-sm z-10">
                          <div className="relative">
                            ...
                            <input
                              ...
                              className="w-full h-8 pl-8 pr-3 text-xs rounded-md bg-muted/50 border-none outline-none focus:ring-1 focus:ring-blue-500/50 text-foreground"
                            />
                          </div>
                        </div>
                ...
                <PhoneInput
                  ...
                  style={{ backgroundColor: 'var(--cta-input-bg)' }}
                  className="flex-1 h-14 px-5 border-none rounded-2xl text-foreground placeholder:text-muted-foreground/40 focus:ring-1 focus:ring-blue-500/20 transition-all outline-none bg-transparent w-full font-medium"
                />
              </div>
              ...
              <textarea 
                ...
                style={{ backgroundColor: 'var(--cta-input-bg)' }}
                className="w-full min-h-[140px] p-5 border-none rounded-2xl text-foreground resize-none placeholder:text-muted-foreground/40 focus:ring-1 focus:ring-blue-500/20 transition-all outline-none" 
              />
